...
I used Claude Code as a pair-programmer for the implementation work  scaffolding files, wiring up boilerplate, suggesting hook structures, and generating the styled-components blocks. I drove product direction, architecture decisions, and scope, then reviewed every change before it landed. Below is the log of the meaningful prompts and decisions, paraphrased for brevity.

The submission would not be what it is without my judgment on what to build, how to frame it, what to cut, and what was worth pushing further. The AI saved me typing time. It did not pick the features, the stack, the tone, or the trade-offs.
...
## Phase 2  Architecture decisions I made

These are the calls that shaped the whole project. Each is mine; the assistant suggested code to implement them.
...
- **Wrap `usePets` in a `PetsProvider`** so the gallery, detail, selected, and toolbar views share one fetch. Avoids the obvious N+1 trap if each view fetched independently.
...
- Build the gallery page with debounced search, four sort modes, responsive 1/2/4 column grid, and IntersectionObserver-based infinite scroll.
...
I asked: "What additions would actually demonstrate front-end strength beyond the minimum spec?" After looking at the options I picked these four:
...
Decisions I made inside this round:
- The keybinding set itself (vim-style `j k h l` alongside arrows; `?` for help; `x`/Space for toggle). Picked for consistency with GitHub, Gmail, and Vim muscle memory.
...
- Hand-rolled a11y, not `react-aria` or `radix-ui`.
...
After looking at the host company's product, I asked the assistant to reframe the About page and README around enterprise-asset-management vocabulary. On review I stripped that back  it read as a try-hard cover letter and weakened the submission. The work should be the pitch; explaining the pitch out loud undermines it.

This kind of judgment call  recognizing when "more" becomes "too much"  is not something the assistant did on its own. It needed the pushback.
...
- Pick the stack.
- Decide what to cut.
- Recognize when copy had drifted into pitch.
- Make architecture trade-offs without being given the trade-offs to evaluate.
